...
impl Student {
    ...
    pub fn calculate_gpa(&self) -> f32 {
        ...
        let total_quality_points: f32 = self.grades.iter().map(CourseGrade::quality_points).sum();
        let total_credits: u16 = self.grades.iter().map(|course| course.credits).sum();
...
impl StudentDatabase {
    ...
    pub fn average_gpa(&self) -> f32 {
        ...
        let total_gpa: f32 = self.students.values().map(Student::calculate_gpa).sum();
